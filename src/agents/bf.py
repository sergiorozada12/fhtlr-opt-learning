...
from src.models import LinearModel,FHLinearModel
...
class Linear:
    def __init__(
        ...
        self.Q = LinearModel(
            len(discretizer.bucket_states), np.prod(discretizer.bucket_actions)
        ).double()
...
class FHLinear:
    def __init__(
        ...
        self.buffer = ReplayBuffer(buffer_size)
        self.discretizer = discretizer
        self.Q = FHLinearModel(
            len(discretizer.bucket_states), np.prod(discretizer.bucket_actions), H
        ).double()
        self.opt = Adamax(self.Q.parameters(), lr=alpha)
