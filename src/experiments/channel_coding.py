...
def run_experiment_with_agent(agent_name, n_exp):
    ...
    if agent_name == "ctrl_ucbm":
        agent = CTRLUCBM(
            DISCRETIZER, H, gamma=GAMMA, actor_lr=ALPHA_CTRL,
            exact_action_threshold=32,
        )
    if agent_name == "lsvi_ucb":
        agent = FHLSVIUCB(
            DISCRETIZER, H, bonus_coef=1.0, feature_seed=n_exp,
            normalize_states=True, action_candidates=256,
        )
    
    if agent_name == "pg":
        Gs = run_pg_experiment(n=n_exp, E=EPISODES, H=H, env=generate_env(), agent=agent)
...
def run_pg_channel_coding_simulations(n_exps=100, delta_exp=0):
    run_paralell(['pg'], ['pg'], n_exps, delta_exp)


def run_ctrl_ucbm_channel_coding_simulations(n_exps=10, delta_exp=0):
    run_paralell(['ctrl_ucbm'], ['ctrl_ucbm'], n_exps, delta_exp)


def run_lsvi_ucb_channel_coding_simulations(n_exps=100, delta_exp=0):
