...
def run_experiment_with_agent(agent_name, n_exp):
    ...
    if agent_name == "lsvi_ucb":
        agent = FHLSVIUCB(DISCRETIZER, H, bonus_coef=1.0, feature_seed=n_exp, normalize_states=True)
...
def run_pg_wireless_simulations(n_exps=100, delta_exp=0):
    run_paralell(['pg'], ['pg'], n_exps, delta_exp)


def run_ctrl_ucbm_wireless_simulations(n_exps=10, delta_exp=0):
    run_paralell(['ctrl_ucbm'], ['ctrl_ucbm'], n_exps, delta_exp)


def run_lsvi_ucb_wireless_simulations(n_exps=100, delta_exp=0):
